...
public class People implements Serializable {
    ...
    public People(SQLiteDatabase db, Zipcode zipcode)
    {
        try {
            ...
            for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
                ...
                persons.add(new Person(id, fname, lname, addr, zipcode, phone, mail,
                        date, title, text));
            }
            cursor.close();
        }
                  ...
                  String persontitle)
    {
        try {
            ...
            for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext())
            {
                ...
                String title = cursor.getString(DbHelper.ACOLUMN_TITLE);
                String text = cursor.getString(DbHelper.ACOLUMN_TEXT);
                persons.add(new Person(id, fname, lname, addr, getZipcode(db, code), phone, mail,
                        date, title, text));
            }
            cursor.close();
        }
    ...
    private Zipcode getZipcode(SQLiteDatabase db, String code)
    {
        Cursor cursor = db.query(DbHelper.ZTABLE_NAME, DbHelper.ZTABLE_COLUMNS,
                ...
                null, null, null);
        cursor.moveToFirst();
        return new Zipcode(cursor.getString(DbHelper.ZCOLUMN_CODE),
                cursor.getString(DbHelper.ZCOLUMN_CITY));
    }
}
